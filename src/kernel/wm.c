...
void wm_render(void) 
{
    ...
    assign_rects(root, 0, 0, screen_w, screen_h);
...
{
    return windows[focused_slot];
}

void wm_focused_rect(uint32_t *x, uint32_t *y, uint32_t *w, uint32_t *h) 
{
    if (win_count == 0 || !window_used[focused_slot]) {
        *x = *y = *w = *h = 0;
        return;
    }
    ...
    *x = nodes[leaf].x + GAP;
    *y = nodes[leaf].y + GAP;
    *w = (nodes[leaf].w > 2 * GAP) ? nodes[leaf].w - 2 * GAP : 0;
    *h = (nodes[leaf].h > 2 * GAP) ? nodes[leaf].h - 2 * GAP : 0;
}
