...
/* Process a raw PS/2 scancode (set 1).
   Returns the encoded character (with KB_CTRL/KB_ALT in high bits)
   or -1 if no character event (modifier change, break, extended prefix).
   Auto-repeat is suppressed: only the initial press generates a character. */
static int ps2_process(unsigned char sc)
{
    if (sc == 0xE0) { ext = 1; return -1; }
    if (sc == 0xE1) { ext = 2; return -1; }

    int make = !(sc & 0x80);
    unsigned char code = sc & 0x7F;

    if (ext == 2) { ext = 0; return -1; }

    if (ext) {
        ext = 0;
        if (code == 0x1C && make) return '\n';
        if (code == 0x38) { alt = make; return -1; }
        if (code == 0x1D) { ctrl = make; return -1; }
        return -1;
    }

    if (code == 0x2A || code == 0x36) { shift = make; return -1; }
    if (code == 0x1D) { ctrl = make; return -1; }
    if (code == 0x38) { alt = make; return -1; }
    if (code == 0x3A && make) { caps = !caps; return -1; }

    int idx = ext ? (code | 0x80) : code;
    if (make) {
        if (key_down[idx]) return -1;
        key_down[idx] = 1;
    } else {
        key_down[idx] = 0;
    }
...
static int ps2_poll_one(void)
{
    if (inb(PS2_STAT) & 1) {
        unsigned char sc = inb(PS2_DATA);
        return ps2_process(sc);
    }
    return -1;
}
...
int kb_getc(void)
{
    /* 1. Ring buffer from PS/2 interrupt handler */
    if (head != tail) {
        int c = ring[tail];
        tail = (tail + 1) % RING_SZ;
        return c;
    ...
    /* 2. UEFI ConIn (primary on UEFI hw without real PS/2) */
    if (conin) {
    ...
    /* 3. Direct PS/2 poll (last resort — on some hw this triggers SMM to stop USB legacy) */
...
void kb_init(void)
{
    ...
    for (int i = 0; i < 256; i++) key_down[i] = 0;

    /* Do NOT touch 8042 ports (0x60/0x64) here — on UEFI systems with
       USB Legacy Support, any access can trigger the firmware's SMM handler
       to stop routing USB keyboard data through the PS/2 emulation.
       Input will come from UEFI ConIn (if available) or from the interrupt
       handler + direct polling in kb_getc(). */

    kb_reload_layout();
    serial_printf("LOG: KB: init done (no PS/2 touch)\n");
}
